...
void FitnessFunction(int curIterNum, int maxIterNum, vector<BestPathInfo>& bestPathInfoList, ProblemParas proParas, Individual& individual);
...
void FitnessFunction(int curIterNum, int maxIterNum, vector<BestPathInfo>& bestPathInfoList, ProblemParas proParas, Individual& individual)
{
	...
	double deviceDist = 0;
	individual.objectiveSet[0] = individual.objectiveSet[1] = 0;
	...
	for (int i = 0; i < individual.geneSize; i += 3) {
		...
		double firstLowX = individual.genes[i] - outSizeLength;
		double firstUpX = individual.genes[i] + outSizeLength;
		double firstLowY = individual.genes[i + 1] - outSizeWidth;
		double firstUpY = individual.genes[i + 1] + outSizeWidth;
		for (int j = i + 3; j < individual.geneSize; j += 3) {
			...
			double secondLowX = individual.genes[j] - outSizeLength;
			double secondUpX = individual.genes[j] + outSizeLength;
			double secondLowY = individual.genes[j + 1] - outSizeWidth;
			double secondUpY = individual.genes[j + 1] + outSizeWidth;
			if (IsRangeOverlap(firstLowX, firstUpX, secondLowX, secondUpX) && IsRangeOverlap(firstLowY, firstUpY, secondLowY, secondUpY)) {
				//individual.objectiveSet[0] = individual.objectiveSet[1] = MAX_FITNESS;
	...
	if (IsDeviceOverlap == true) {
		...
		for (int i = 0; i < individual.geneSize; ++i) {
			particlePosList.push_back(individual.genes[i]);
		}
		for (int i = 0; i < proParas.DeviceSum; ++i) {
		...
		if ((max_X - min_X) <= proParas.workShopLength
			...
			//修改postion，需要修改其他的吗？
			for (int i = 0; i < proParas.DeviceSum; i++) {
				individual.genes[i * 3] = particlePosList[3 * i];
				individual.genes[i * 3 + 1] = particlePosList[3 * i + 1];
			}
		}
		else {
			IsDeviceOverlap = true;
	...
	if (IsDeviceOverlap == false)
	{
		...
		//		if (i != j)
		//		{
		//			if (abs(individual.genes[3 * i] - individual.genes[3 * j]) <= 1)
		//			{
		//				individual.genes[3 * i] = individual.genes[3 * j]
		//					= (individual.genes[3 * i] + individual.genes[3 * j]) * 0.5f;
		//			}
		//			if (abs(individual.genes[3 * i + 1] - individual.genes[3 * j + 1]) <= 1)
		//			{
		//				individual.genes[3 * i + 1] = individual.genes[3 * j + 1]
		//					= (individual.genes[3 * i + 1] + individual.genes[3 * j + 1]) * 0.5f;
		//			}
		//		}
		//	}
		//}
		...
		//for (int i = 0; i < proParas.DeviceSum; i++)
		//{
		//	if (abs(individual.genes[3 * i] - proParas.entrancePos.x) <= 1)
		//	{
		//		individual.genes[3 * i] = proParas.entrancePos.x;
		//	}
		//	if (abs(individual.genes[3 * i + 1] - proParas.entrancePos.y) <= 1)
		//	{
		//		individual.genes[3 * i + 1] = proParas.entrancePos.y;
		//	}
		//}
		////}
		#pragma endregion
		...
		for (int j = 0; j < proParas.CargoTypeNum; j++)
		{
			for (int k = 0; k < proParas.cargoTypeList[j].linkSum; k++)
			{
				...
				if (outDeviceIndex == -1)
				{
					...
					Vector2 inPointTPos(inPoint.pos.x + individual.genes[inDeviceIndex * 3],
						inPoint.pos.y + individual.genes[inDeviceIndex * 3 + 1]);
					if (inPointTPos.x != proParas.entrancePos.x && abs(inPointTPos.x - proParas.entrancePos.x) < proParas.conveyMinDist)
					{
						...
						individual.genes[inDeviceIndex * 3] -= moveLength;
					...
					else if (inPointTPos.y != proParas.entrancePos.y && abs(inPointTPos.y - proParas.entrancePos.y) < proParas.conveyMinDist)
					{
						...
						individual.genes[inDeviceIndex * 3 + 1] -= moveLength;
					}
				}
				else if (inDeviceIndex == -2)//特殊情况2：仓库出口
				{
					outPoint = copyDeviceParas[outDeviceIndex].adjPointsOut[outPointIndex];
					Vector2 outPointTPos(outPoint.pos.x + individual.genes[outDeviceIndex * 3],
						outPoint.pos.y + individual.genes[outDeviceIndex * 3 + 1]);
					if (outPointTPos.x != proParas.exitPos.x && abs(outPointTPos.x - proParas.exitPos.x) < proParas.conveyMinDist)
					{
						...
						individual.genes[outDeviceIndex * 3] -= moveLength;
					}
					else if (outPointTPos.y != proParas.exitPos.y && abs(outPointTPos.y - proParas.exitPos.y) < proParas.conveyMinDist)
					{
						...
						individual.genes[outDeviceIndex * 3 + 1] -= moveLength;
					}
				}
				else//其他情况
				{
			...
			for (int k = 0; k < proParas.cargoTypeList[j].linkSum; k++)
			{
				...
				else//其他情况
				{
					...
					Vector2 inPointTPos(inPoint.pos.x + individual.genes[inDeviceIndex * 3],
						inPoint.pos.y + individual.genes[inDeviceIndex * 3 + 1]);
					//出入口设备四个方向不考虑外层的边界
					Vector2 outDeviceUpPos(individual.genes[outDeviceIndex * 3], individual.genes[outDeviceIndex * 3 + 1] + 0.5 * copyDeviceParas[outDeviceIndex].size.y);
					Vector2 outDeviceDownPos(individual.genes[outDeviceIndex * 3], individual.genes[outDeviceIndex * 3 + 1] - 0.5 * copyDeviceParas[outDeviceIndex].size.y);
					Vector2 outDeviceLeftPos(individual.genes[outDeviceIndex * 3] - 0.5 * copyDeviceParas[outDeviceIndex].size.x, individual.genes[outDeviceIndex * 3 + 1]);
					Vector2 outDeviceRightPos(individual.genes[outDeviceIndex * 3] + 0.5 * copyDeviceParas[outDeviceIndex].size.x, individual.genes[outDeviceIndex * 3 + 1]);

					Vector2 inDeviceUpPos(individual.genes[inDeviceIndex * 3], individual.genes[inDeviceIndex * 3 + 1] + 0.5 * copyDeviceParas[inDeviceIndex].size.y);
					Vector2 inDeviceDownPos(individual.genes[inDeviceIndex * 3], individual.genes[inDeviceIndex * 3 + 1] - 0.5 * copyDeviceParas[inDeviceIndex].size.y);
					Vector2 inDeviceLeftPos(individual.genes[inDeviceIndex * 3] - 0.5 * copyDeviceParas[inDeviceIndex].size.x, individual.genes[inDeviceIndex * 3 + 1]);
					Vector2 inDeviceRightPos(individual.genes[inDeviceIndex * 3] + 0.5 * copyDeviceParas[inDeviceIndex].size.x, individual.genes[inDeviceIndex * 3 + 1]);
					...
					//先判断两者的方位
					//加一个判断二者方位的操作
					Vector2 inDevicePos(individual.genes[inDeviceIndex * 3], individual.genes[inDeviceIndex * 3 + 1]);
					Vector2 outDevicePos(individual.genes[outDeviceIndex * 3], individual.genes[outDeviceIndex * 3 + 1]);
					...
					case 1://上上
					{
						...
						if (outPointTPos.y != inPointTPos.y && abs(outPointTPos.y - inPointTPos.y) < proParas.conveyMinDist)
						{
							...
							individual.genes[OutPosIndex_Y] -= moveLength;
							individual.genes[InPosIndex_Y] += moveLength;
							break;
						}
						//2.入口设备在出口设备左/右上角
						...
						if (inUp2OutBool)
						{
							...
							if (inR2outXDist > 0 && inR2outXDist < proParas.conveyMinDist)
							{
								...
								individual.genes[OutPosIndex_X] += moveLength;
								individual.genes[InPosIndex_X] -= moveLength;
								break;
							}
		...
		for (int i = 0; i < proParas.DeviceSum; i++)
		{
			for (AdjPoint point : copyDeviceParas[i].adjPointsIn)
			{
				...
				Vector2 axis(point.pos.x + individual.genes[3 * i], point.pos.y + individual.genes[3 * i + 1]);
		...
		for (int i = 0; i < proParas.DeviceSum; i++)
		{
			for (AdjPoint p : copyDeviceParas[i].adjPointsIn)
			{
				...
				{
					horizonAxisList.push_back(p.pos.x + individual.genes[i * 3]);
				}
				else {//左右
					verticalAxisList.push_back(p.pos.y + individual.genes[i * 3 + 1]);
				}
			}
		...
		for (int i = 0; i < individual.geneSize; i += 3) {
			...
			double LowX = individual.genes[i] - outSizeLength;
			double HighX = individual.genes[i] + outSizeLength;
			double LowY = individual.genes[i + 1] - outSizeWidth;
			double HighY = individual.genes[i + 1] + outSizeWidth;
			...
			//防止路径进入设备内部
			horizonAxisList.push_back(individual.genes[i]);
			verticalAxisList.push_back(individual.genes[i + 1]);
		...
		//vector<PointLink>().swap(individual.pointLinks);
		...
		for (int i = 0; i < proParas.CargoTypeNum; i++)
		{
			...
			for (int j = 0; j < proParas.cargoTypeList[i].linkSum; j++)//遍历物料经过的设备列表
			{
				...
				if (forwardDeviceIndex == -1)//说明是仓库入口
				{
					...
					device2PosX = copyDeviceParas[curDeviceIndex].adjPointsIn[curInIndex].pos.x + individual.genes[curDeviceIndex * 3];
					device2PosY = copyDeviceParas[curDeviceIndex].adjPointsIn[curInIndex].pos.y + individual.genes[curDeviceIndex * 3 + 1];
				...
				else if (curDeviceIndex == -2)//说明是出口
				{
					...
					device1PosX = copyDeviceParas[forwardDeviceIndex].adjPointsOut[forwardOutIndex].pos.x + individual.genes[forwardDeviceIndex * 3];
					device1PosY = copyDeviceParas[forwardDeviceIndex].adjPointsOut[forwardOutIndex].pos.y + individual.genes[forwardDeviceIndex * 3 + 1];
				...
				//			//	individual.fitness_[0] = individual.fitness_[1] = MAX_FITNESS;
				//			//	return;
				//			//}
				//		points.push_back(lastP);
				...
				//individual.pointLinks.push_back(pointLink);
				#pragma endregion
		...
		//individual.fitness_[0] = 1000;
		individual.objectiveSet[0] = totalTime;
		//individual.objectiveSet[0] = totalTime + punishValue1;

		//individual.objectiveSet[1] = CalcuTotalArea(particle, proParas);
		//individual.objectiveSet[1] = 1000;
		individual.objectiveSet[1] = conveyorTotalCost;
		//individual.objectiveSet[1] = conveyorTotalCost + punishValue2;

		//根据适应度是否升级选择更新BestPathInfoList
		for (int i = 0; i < bestPathInfoList.size(); ++i) {
			if (individual.objectiveSet[i] < bestPathInfoList[i].curBestFitnessVal) {//需要更新
				for (int j = 0; j < individual.geneSize; ++j) {
					bestPathInfoList[i].devicePosList[j] = individual.genes[j];
				}
				...
				bestPathInfoList[i].curBestFitnessVal = individual.objectiveSet[i];
			}
		}
		#pragma endregion
...
double CalcuTotalArea(Individual& individual, ProblemParas proParas) {
	...
	for (int i = 0; i < individual.geneSize; i += 2) {
		if (individual.genes[i] - proParas.deviceParaList[i / 2].size.x * 0.5 < min_X) {
			min_X = individual.genes[i] - proParas.deviceParaList[i / 2].size.x * 0.5;
			min_X_index = i / 2;
		}
		if (individual.genes[i + 1] - proParas.deviceParaList[i / 2].size.y * 0.5 < min_Y) {
			min_Y = individual.genes[i + 1] - proParas.deviceParaList[i / 2].size.y * 0.5;
			min_Y_index = i / 2;
		}
		if (individual.genes[i] + proParas.deviceParaList[i / 2].size.x * 0.5 > max_X) {
			max_X = individual.genes[i] + proParas.deviceParaList[i / 2].size.x * 0.5;
			max_X_index = i / 2;
		}
		if (individual.genes[i + 1] + proParas.deviceParaList[i / 2].size.y * 0.5 > max_Y) {
			max_Y = individual.genes[i + 1] + proParas.deviceParaList[i / 2].size.y * 0.5;
			max_Y_index = i / 2;
		}
	}
	//计算总面积
